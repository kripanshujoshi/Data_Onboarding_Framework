...
def generate_insert_statements(src_nm, dataset_nm, table_nm, df_dataset_info, df_pre_proc_info, df_table_info, df_metadata_df) -> str:
    ...
    if db.check_existence(src_nm, dataset_nm, table_nm):
        update_statements.append(create_update_statement("app_mgmt.sys_config_dataset_info", df_dataset_info, ["src_nm", "dataset_nm"]))
        update_statements.append(create_update_statement("app_mgmt.sys_config_pre_proc_info", df_pre_proc_info, ["src_nm", "dataset_nm"]))
        update_statements.append(create_update_statement("app_mgmt.sys_config_table_info", df_table_info, ["src_nm", "dataset_nm"]))
        update_statements.append(create_update_statement("app_mgmt.sys_config_table_field_info", df_metadata_df, ["src_nm", "src_table_nm"]))

        st.session_state.rds_sql_script = "\n\n".join(update_statements)
    
    else:
        insert_statements.append(create_insert_statement("app_mgmt.sys_config_dataset_info", df_dataset_info))
        insert_statements.append(create_insert_statement("app_mgmt.sys_config_pre_proc_info", df_pre_proc_info))
        insert_statements.append(create_insert_statement("app_mgmt.sys_config_table_info", df_table_info))
        insert_statements.append(create_insert_statement("app_mgmt.sys_config_table_field_info", df_metadata_df))
