...
with col1:
    src_nm = st.text_input("Source Name", placeholder="e.g. fin_user, smrt_fctry, comrcl_user")
    fmt_type_cd = st.selectbox("Format Type", ["csv", "excel", "txt", "zip", "json", "parquet", "orc"])
with col2:
    domn_nm = st.text_input("Domain Name", placeholder="e.g. sales_prfmnc_eval, mktg_start_plan, info_sec_mgmt")
    delmtr_cd = st.text_input("Delimiter Code")
with col3:
    dataset_nm = st.text_input("Dataset Name", placeholder="e.g. sales_prfmnc_eval, mktg_start_plan, info_sec_mgmt")
    dprct_methd_cd = st.selectbox("Deprecation Method", ["okrdra", "orrrra", "okkkra", "okkkra_extrctr", "okkkka", "file_cntl_upsrt_repl"])
with col4:
    table_nm = st.text_input("Table Name")
    data_clasfctn_nm = st.selectbox("Data Classification Name", ["confd", "intrnl"])
with col5:
    dialect = st.selectbox("Target Warehouse", ["Snowflake", "Redshift"])
    warehouse_nm = st.selectbox("Compute Role", ["keu_it_small", "keu_fin_small", "keu_elt_analytic_small", "keu_elt_supplychain_small"])

uploaded_file = st.file_uploader("Upload File (CSV, ZIP, or Excel)", type=["xlsx", "csv", "zip"])
...
if generate_template:
    ...
    try:
        if uploaded_file is None:
            ...
            st.warning("Existing configurations found. Kindly validate before proceeding.")
...
if st.session_state.template_generated:
    if db.check_existence(src_nm, dataset_nm, table_nm):
        st.session_state.metadata_df = onboarding_service.get_table_field_info(src_nm, table_nm)
        st.session_state.sys_config_dataset_info = onboarding_service.get_dataset_info(src_nm, dataset_nm)
        st.session_state.sys_config_pre_proc_info = onboarding_service.get_pre_proc_info(src_nm, dataset_nm)
        st.session_state.sys_config_table_info = onboarding_service.get_table_info(src_nm, dataset_nm)
    else:
        metadata_df, sys_config_dataset_info, sys_config_pre_proc_info, sys_config_table_info = onboarding_service.generate_templates(
            uploaded_file, src_nm, domn_nm, dataset_nm, table_nm, data_clasfctn_nm, fmt_type_cd, delmtr_cd, dprct_methd_cd, dialect, warehouse_nm
        )
    ...
    sheet_names = ["sys_config_dataset_info", "sys_config_pre_proc_info", "sys_config_table_info", "sys_config_table_field_info"]
    ...
    with tab1:
        ...
        dataset_trgt_dw_list = ['snowflake', 'redshift', 'redshift|snowflake']
    ...
    # with tab1:
    #     st.write("### sys_config_dataset_info")
    #     st.data_editor(st.session_state.sys_config_dataset_info, num_rows="dynamic", key="sys_config_dataset_info_edit")
    # with tab2:
    #     st.write("### sys_config_pre_proc_info")
    #     st.data_editor(st.session_state.sys_config_pre_proc_info, num_rows="dynamic", key="sys_config_pre_proc_info_edit")
    # with tab3:
    #     st.write("### sys_config_table_info")
    #     st.data_editor(st.session_state.sys_config_table_info, num_rows="dynamic", key="sys_config_table_info_edit")
    # with tab4:
    #     st.write("### sys_config_table_field_info")
    #     st.data_editor(st.session_state.metadata_df, num_rows="dynamic", key="sys_config_table_field_info_edit")
    ...
    if generate_sql_button:
        ...
        try:
            if st.session_state.metadata_df.empty:
                ...
                logger.warning("Metadata DataFrame is empty! SQL cannot be generated.")
            else:
...
if st.session_state.sql_generated:
    ...
    if insert_into_rds_clicked:
        ...
        try:
            success = onboarding_service.insert_into_rds(st.session_state.rds_sql_script)
            if success:
